...
void main() {
  ...
  group('ProjectController', () {
    ...
    test('handleServerMessage handles snapshot', () async {
      final nodeData = NodeModel(
        id: 'snap_1',
        type: 'rect',
        pos: const Vec2(1, 2),
      ...
      mockChannel!.simulateIncoming(
        jsonEncode({'type': 'snapshot', 'nodes': [nodeData]}),
      );
      await Future.delayed(Duration.zero);

      expect(pc.engine.nodes.length, 1);
      expect(pc.engine.nodes.first.id, 'snap_1');
    });
    ...
    test('handleServerMessage handles presence_update', () async {
      mockChannel!.simulateIncoming(
        jsonEncode({
      ...
      expect(presence.remoteUsers.containsKey('remote_user'), true);
      expect(presence.remoteUsers['remote_user']!.cursor.value.x, 12.0);
    });
    ...
    test('connection error does not crash', () async {
      mockChannel!.simulateError(Exception('boom'));
      await Future.delayed(Duration.zero);
      // LxStream should emit LxError internally; just ensure no crash.
      expect(true, true);
    });
    ...
    test('endDrag tolerates missing node ids', () async {
      pc.addNode('rect');
      pc.addNode('rect2');

      final draggedId = pc.engine.nodes.first.id;
      pc.selectedIds.assignOne(draggedId);

      pc.startDrag();
      // Remove the node after capturing drag start positions to trigger fallback logic.
      pc.engine.removeNode(draggedId);
      pc.endDrag();

      expect(pc.selectedIds.contains(draggedId), true);
    });

    test('export errors on empty project', () async {
      final emptyChannel = FakeWebSocketChannel();
      final emptyPc = ProjectController(
      ...
      expect(emptyPc.exportStatus.value, isNotNull);
      expect(emptyPc.exportStatus.value!.status, isA<LxError>());
    });
  });
  ...
  group('PresenceController', () {
    ...
    test('removeUser removes remote user', () {
      ...
      presence.removeUser('other_user');
      expect(presence.remoteUsers.containsKey('other_user'), false);
    });
