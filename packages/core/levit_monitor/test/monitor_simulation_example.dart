...
class LocationController extends LevitController {
  ...
  final location = locationStream.stream.lx.named('location');

  /// Internal mutable reactive collection
  final wayPoints = LxList<(int, int)>().named('wayPoints');
  ...
  Future<void> onInit() async {
    ...
    ///
    /// The `.listen()` shorthand creates an LxWorker that is explicitly
    /// registered and automatically disposed with the controller lifecycle.
    autoDispose(
      location
          .transform((s) => s.sampleTime(const Duration(seconds: 1)))
          .listen((_) => addWayPoint())
          .named('watch'),
    );
  }
}
