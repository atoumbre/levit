...
class _Disposable extends LevitScopeDisposable {
  ...
  @override void onClose() => _onClose();
}

void main() {
  test('lazyPutAsync disposed during init throws and closes instance', () async {
    final scope = LevitScope.root('root3');
    int closed = 0; final completer = Completer<void>();

    scope.lazyPutAsync<_Disposable>(() async { await completer.future; return _Disposable(() => closed++); });
