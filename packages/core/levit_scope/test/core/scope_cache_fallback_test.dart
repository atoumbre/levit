...
void main() {
  group('Resolution cache fallback', () {
    test('find(tag) falls back when cached scope throws', () {
      final root = LevitScope.root('root'); root.put<String>(() => 'root', tag: 't');
      final child = root.createScope('child'); final unrelated = LevitScope.root('unrelated');
    ...
    test('findOrNull(tag) falls back when cached scope recurses', () {
      final root = LevitScope.root('root_or_null'); root.put<String>(() => 'root', tag: 't');
...
void _setResolutionCache(LevitScope scope, LevitScopeKey key, LevitScope cached) {
  ...
  final cache = mirror.getField(symbol).reflectee as Map<LevitScopeKey, LevitScope>;
  cache[key] = cached;
}
