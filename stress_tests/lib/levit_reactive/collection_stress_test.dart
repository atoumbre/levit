...
void main() {
  group('Stress Test: Collections', () {
    test('LxList Bulk Assign - 1M items', () {
      ...
      final list = <int>[].lx;
      final largeData = List.generate(5000000, (i) => i);

      final sw = Stopwatch()..start();
      list.value = largeData;
      sw.stop();

      expect(list.length, 5000000);
      print('Assigned 5M items to LxList in ${sw.elapsedMilliseconds}ms');

      list.close();
    });

    test('LxList Mutation Burst - 500k random ops', () {
    ...
    test('LxMap Bulk Insert - 1M entries', () {
    ...
    test('LxMap Update Flood - 100k key updates', () {
